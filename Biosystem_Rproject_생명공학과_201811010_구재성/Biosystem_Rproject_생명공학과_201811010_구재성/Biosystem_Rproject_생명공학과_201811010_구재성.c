...
/*Dataset을 바꾸려면 아래 기호상수에서 다른 파일로 교체하시오.
ex)"dataset/Data1_FP1.txt" = >"dataset/Data2_FP1.txt", "dataset/Data2_FP2.txt" = >"dataset/Data2_FP2.txt"
Data1부터 Data10까지 가능함*/
#define eeg1 "dataset/Data1_FP1.txt"
#define eeg2 "dataset/Data1_FP2.txt"
...
int music_1() {
	...
	int key;
	PlaySound(TEXT("music/백색소음.wav"), NULL, SND_ASYNC || SND_LOOP);
	while (1)
	{
...
int music_2() {
	...
	int key;
	PlaySound(TEXT("music/클래식.wav"), NULL, SND_ASYNC || SND_LOOP);
	while (1)
	{
		printf("  종료하려면 아무키나 입력하세요!  ");
		key = getch();
...
int music_3() {
	...
	int key;
	PlaySound(TEXT("music/명상음악.wav"), NULL, SND_ASYNC || SND_LOOP);
	while (1)
	{
		printf("  종료하려면 아무키나 입력하세요!  ");
		key = getch();
...
void main()
{
	...
	for (i = 0; i < 256; i++) {
		data_3[i] = (data_1[i] + data_2[i]) / 2 - average;
	}
	for (i = 0; i < 256; i++) {
		average += data_3[i] / 256.0;
	}
